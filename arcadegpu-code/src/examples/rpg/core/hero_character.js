...
class HeroCharacter extends CharacterAbstract {
  ...
  async loadFromData(data) {
    if (data['WeaponId']) {
      ...
      await weapon.loadFromFile('examples/rpg/data/' + data['WeaponId'] + '/data.json');
      this.setEquipment(weapon);
    }
    if (data['HelmetId']) {
      let helmet = new EquipmentItem();
      await helmet.loadFromFile('examples/rpg/data/' + data['HelmetId'] + '/data.json');
      this.setEquipment(helmet);
    }
    if (data['ArmorId']) {
      let armor = new EquipmentItem();
      await armor.loadFromFile('examples/rpg/data/' + data['ArmorId'] + '/data.json');
      this.setEquipment(armor);
    }
    if (data['RelicId']) {
      let relic = new EquipmentItem();
      await relic.loadFromFile('examples/rpg/data/' + data['RelicId'] + '/data.json');
    ...
    this.cast.loadFromFile('examples/rpg/data/cast_' + data['CastId'] + '/data.json');
