...
export async function spawnPlayer(name) {
  ...
  await playerJAS.loadFromFile('examples/fight/chars/' + name + '/sprite.jas');
  playerJAS.setTexture(await gfx2TextureManager.loadTexture('examples/fight/chars/' + name + '/sprite.png'));
  playerJAS.setOffset(+44, +44);

  const damageJAS = new Gfx2SpriteJAS();
  await damageJAS.loadFromFile('examples/fight/chars/' + name + '/sprite.jas');
  damageJAS.setTexture(await gfx2TextureManager.loadTexture('examples/fight/chars/' + name + '/sprite.png'));
