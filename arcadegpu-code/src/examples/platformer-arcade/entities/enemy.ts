...
export const spawnEnemy = async (x: number, y: number) => {
  ...
  await sprite.loadFromFile('examples/platformer/enemy.jas');
  sprite.setTexture(await gfx2TextureManager.loadTexture('examples/platformer/enemy.png'));  
...
export const spawnExplosion = async (x: number, y: number) => {
  const sprite = new Gfx2SpriteJAS();
  await sprite.loadFromFile('examples/platformer/explosion.jas');
  sprite.setTexture(await gfx2TextureManager.loadTexture('examples/platformer/explosion.png'));  
