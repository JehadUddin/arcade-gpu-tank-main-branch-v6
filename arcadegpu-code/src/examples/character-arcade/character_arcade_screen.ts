...
class CharacterArcadeScreen extends Screen {
  mapJSM: Gfx3MeshJSM;
  ...
  async #createMap(): Promise<{jsm: Gfx3MeshJSM, jnm: Gfx3PhysicsJNM }> {
    ...
    await jsm.loadFromFile('./examples/third-person/map.jsm');
    jsm.mat.setTexture(await gfx3TextureManager.loadTexture('./examples/third-person/map.png', {
      magFilter: 'nearest',
      minFilter: 'nearest'
    }));

    const jnm = new Gfx3PhysicsJNM();
    await jnm.loadFromFile('./examples/third-person/map.jnm');
    return { jsm, jnm };
  }
}

export { CharacterArcadeScreen };